...
const E2E_DB_PATH = path.join(ROOT_DIR, 'e2e', 'e2e-test.db');
...
async function main() {
  ...
  const resultsDir = path.join(ROOT_DIR, 'e2e', 'results');
  ...
  const testProc = spawn('npx', ['playwright', 'test', '--config', 'e2e/playwright.config.js'], {
    cwd: ROOT_DIR,
    shell: false,
    stdio: 'inherit',
  ...
  if (exitCode !== 0) {
    ...
    console.log('\n❌ E2E tests failed.');
    console.log('   Report: e2e/results/last-failure-report.md');
    console.log('   Screenshots: e2e/results/test-results/');
    process.exit(1);
  }
